...
public class ScriptThread implements Runnable {
    ...
    */
    ...
    private static final int CMD_INDEX_EOF = -1;
    ...
    public void run() {
        ...
        while (true) {
            try {
                String msg = TCPServerThread.getMessage();
    ...
    public static int getEndOfFileID() {
        return CMD_INDEX_EOF;
    }
    
    /**
     * returns the max length of a String to send to client.
     * 
     * @param line - the message to sned
     * 
     * @return max length of String
     */
    public static String limitNetworkString (String line) {
        if (line.length() > MAX_STR_LENGTH) {
            String addendum = "... (len = " + line.length() + ")";
            line = line.substring(0, MAX_STR_LENGTH - addendum.length()) + addendum;
        }
        return line;
    }
    ...
    /**
     * This performs the RESET actions on the classes.
     * This handles the resetting of all the classes involved in state changes
     *  during execution.
     */
     ...
     * This is called when the RESER button is pressed to reset the execution
     *  to its startup conditions.
     */
    ...
    private static void setBreakpoint (String value) {
        if (value.contentEquals("OFF")) {
            ...
            int minline = ScriptCompile.getMinLineNumber();
            int maxline = ScriptCompile.getMaxLineNumber();
            breakIndex = ScriptCompile.getCommandIndex(line);
            if (breakIndex == CMD_INDEX_EOF) {
                if (line < 1 || line > maxline) {
                    GUILogPanel.outputInfoMsg(MsgType.WARN, "Breakpoint value " + value + " exceeded valid range of " + minline + " to " + maxline);
                    TCPServerThread.sendStatus("BREAKPT ERROR");
                } else {
                    GUILogPanel.outputInfoMsg(MsgType.WARN, "Breakpoint value " + value + " not an executable statement");
                    TCPServerThread.sendStatus("BREAKPT INVALID");
                }
                return;
            }
